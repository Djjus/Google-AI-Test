...
const rawData = `Order Number,Product,Price,Date,Payment Method
...
export const orders: Order[] = rawData
  .split('\n')
  .slice(1)
  .map((line) => {
    const [id, product, priceStr, date, paymentMethod] = line.split(',');
    return {
      id,
      product,
...
export const getAggregatedDailyRevenue = () => {
  const revenueMap = new Map<string, number>();
  orders.forEach((order) => {
    revenueMap.set(order.date, (revenueMap.get(order.date) || 0) + order.price);
  });
...
export const getProductPerformance = () => {
  const performanceMap = new Map<string, { totalRevenue: number; count: number }>();
  orders.forEach((order) => {
    const stats = performanceMap.get(order.product) || { totalRevenue: 0, count: 0 };
    stats.totalRevenue += order.price;
    stats.count += 1;
    performanceMap.set(order.product, stats);
  });
